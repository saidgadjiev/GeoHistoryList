...
namespace GeoList
{
	public class GeoListAdapter : RecyclerView.Adapter
	{
		...
		public override void OnBindViewHolder (RecyclerView.ViewHolder holder, int position)
		{
			...
			geoListHolder.latitude.Text = string.Format ("{0:f3}", _geoLocaitonList [position].latitude);
			geoListHolder.longitude.Text = string.Format ("{0:f3}", _geoLocaitonList [position].longitude);
		}

		public override RecyclerView.ViewHolder OnCreateViewHolder (Android.Views.ViewGroup parent, int viewType)
		{
			View view = LayoutInflater.From (parent.Context).Inflate (Resource.Layout.location, parent, false);
